...
function AdminEditor({
  ...
  const handleAutoLocateSpots = useCallback(async () => {
    const candidates = payload.spots.filter(
    ...
    addToast(
      ...
      `自动定位完成`,
      `已定位 ${resolved} 个 / 失败 ${failed} 个${
        failed > 0 ? ',失败的可在 SpotInspector 手动搜索' : ''
      }`,
    );
  ...
  return (
    <div className="admin-shell">
      ...
      <AdminSettingsSheet
        ...
        onAutoLocateSpots={handleAutoLocateSpots}
        isAutoLocating={isAutoLocating}
        missingLocationCount={payload.spots.filter(
          (s) => !Number.isFinite(s.lat) || !Number.isFinite(s.lng),
        ).length}
        isReloading={isReloading}
        isSaving={isSaving}
