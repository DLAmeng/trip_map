...
function tripEditorReducer(state, action) {
    switch (action.type) {
        ...
        case 'INSERT_AFTER_SPOT': {
            ...
            // 修正 nextStopId 链
            const updatedSpots = state.spots.map((s) => s.id === anchor.id ? { ...s, nextStopId: inserted.id } : s);
            ...
            if (oldNextId) {
                const existingSegIdx = state.routeSegments.findIndex((seg) => seg.fromSpotId === anchor.id && seg.toSpotId === oldNextId);
                if (existingSegIdx >= 0) {
                    const existing = state.routeSegments[existingSegIdx];
                    const newSeg = makeBlankSegment({
                        day: anchor.day,
        ...
        case 'SORT_SEGMENTS_BY_DAY': {
            ...
            indexed.sort((a, b) => {
                if (a.segment.day !== b.segment.day)
                    return a.segment.day - b.segment.day;
...
export function useTripEditor(initialData) {
    ...
    const reorderSegments = useCallback((oldIndex, newIndex) => {
