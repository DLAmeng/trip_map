...
export function isTransitTransportType(transportType) {
    if (!transportType)
        return false;
    return ['metro', 'subway', 'train', 'jrrapid', 'shinkansen', 'nankai'].includes(transportType.toLowerCase());
}
export function isRapidApiRailRuntimeEnabled(config) {
    return config.routing?.rapidApi?.enabled === true;
}
export function isRapidApiRailSegmentCached(segment, config) {
    return Boolean(segment.id &&
        config.routing?.rapidApi?.cachedSegmentIds?.includes(segment.id));
}
function isJapanRegion(config) {
    return config.googleMaps?.region?.toUpperCase() === 'JP';
}
export function isJapanTransitHydrationUnsupported(config) {
    // Google 官方 FAQ 标明 Routes API 的 transit 不支持日本合作方，
    // 所以日本区铁路段无法拿到真实 transit polyline。
    return isJapanRegion(config);
}
export function shouldAwaitInitialRailHydration(segment, config) {
    return (isTransitTransportType(segment.transportType) &&
...
function resolveRouteEndpoints(segment, spotById) {
...
function getGoogleRouteTimeoutMs(segment, config) {
...
function extractGoogleRoutePath(route, transportType = '') {
    const points = [];
    const transitOnly = ['metro', 'subway', 'train', 'jrrapid', 'shinkansen', 'nankai'].includes(transportType.toLowerCase());
...
function maybeAttachAccessPoint(points, point, position) {
...
function mergeTransitAccessPoints(segment, resolvedPoints, routingPoints, spotById) {
...
function getGoogleRoutingPoints(segment, spotById, travelMode) {
...
function shouldHydrateRoute(segment, options) {
...
function buildRapidApiRailRequest(segment, spotById, config) {
...
async function fetchRapidApiRailRouteGeometry(segment, spotById, config, signal) {
    ...
    try {
        ...
        const cleanedPath = payload.geometry.path.filter((point) => Array.isArray(point) &&
            point.length >= 2 &&
        ...
        return {
            ...payload.geometry,
            path: mergeTransitAccessPoints(segment, cleanedPath, resolveRouteEndpoints(segment, spotById), spotById),
        };
    }
    catch (error) {
        if (!isAbortError(error)) {
...
async function getGoogleRoutesApi(config) {
    ...
    return google.maps?.routes?.Route || null;
}
...
async function fetchGoogleRouteGeometry(segment, spotById, config, signal) {
    ...
    try {
        ...
        const response = await Promise.race([
            ...
            new Promise((resolve) => {
                timeoutId = window.setTimeout(() => resolve(null), getGoogleRouteTimeoutMs(segment, config));
            }),
        ]);
        if (timeoutId !== null) {
            window.clearTimeout(timeoutId);
...
async function resolveRouteGeometry(segment, spotById, options) {
...
function getHydrationConcurrency(targets, options) {
...
export async function hydrateRealRouteGeometries(options) {
    ...
    await runWithConcurrency(targets, concurrency, async (segment) => {
        ...
        try {
            ...
            if (!geometry || options.signal?.aborted)
                return;
