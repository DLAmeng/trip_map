...
export function parsePathInput(value) {
    if (Array.isArray(value)) {
        return value
            .map((pair) => {
...
export function normalizeSpot(spot) {
    ...
    const type = typeRaw === 'transport' ? 'transport' : 'spot';
    return {
        id: toStr(spot.id),
        ...
        rating: spot.rating === null || spot.rating === undefined
            ? null
...
export function normalizeSegment(segment) {
    ...
    const scope = scopeRaw === 'intercity' ? 'intercity' : 'city';
    let path = [];
...
export function normalizeTripForSave(trip) {
    const issues = [];
