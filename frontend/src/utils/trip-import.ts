...
export interface ParsedImportPoint {
  name: string;
  description: string;
  lat: number;
  lng: number;
  source: 'gpx' | 'kml' | 'google-url';
}

function safeDomParse(text: string): Document | null {
  try {
...
export function parseGpx(text: string): ParsedImportPoint[] {
  const doc = safeDomParse(text);
  if (!doc) return [];
  const result: ParsedImportPoint[] = [];
...
export function parseKml(text: string): ParsedImportPoint[] {
  const doc = safeDomParse(text);
  if (!doc) return [];
  const result: ParsedImportPoint[] = [];
...
export function parseGoogleMapsUrl(url: string): ParsedImportPoint | null {
...
function extractPlaceNameFromGoogleUrl(url: string): string {
...
export async function parseImportFile(file: File): Promise<ParsedImportPoint[]> {
  const text = await file.text();
  const lower = file.name.toLowerCase();
  if (lower.endsWith('.gpx')) return parseGpx(text);
  if (lower.endsWith('.kml') || lower.endsWith('.kmz')) return parseKml(text);
  return [];
}

export function parseImportUrls(urlsText: string): ParsedImportPoint[] {
  return urlsText
    .split('\n')
    .map((s) => s.trim())
    .filter(Boolean)
    .map(parseGoogleMapsUrl)
    .filter((p): p is ParsedImportPoint => p !== null);
}
