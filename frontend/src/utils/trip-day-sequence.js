export function buildCompactDayMap(days) {
    const uniqueDays = Array.from(new Set(days
        .map((day) => Number(day))
        .filter((day) => Number.isFinite(day) && day > 0))).sort((a, b) => a - b);
    return new Map(uniqueDays.map((day, index) => [day, index + 1]));
}
export function compactDayValue(day, dayMap) {
    const numericDay = Number(day);
    if (!Number.isFinite(numericDay) || numericDay <= 0)
        return 1;
    return dayMap.get(numericDay) ?? numericDay;
}
export function compactTripPayloadDays(payload) {
    ...
    const compactedSpots = spots.map((spot) => ({
        ...spot,
        day: compactDayValue(spot.day, dayMap),
    }));
    const compactedSegments = routeSegments.map((segment) => {
        ...
        return {
            ...segment,
            day: compactDayValue(dayMap.has(Number(segment.day)) ? segment.day : fallbackDay, dayMap),
        };
    });
    return {
        ...payload,
